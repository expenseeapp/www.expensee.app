...
import DeletionIcon from '../../../components/DeletionIcon'
...
function Body({ tweet, enableAvatar, full, onClick, onLike, owner }) {
  const router = useRouter()
  const [likes, setLikes] = useState(tweet.likes)
  const [deleteTalk] = useMutation(DELETE_TALK)

  const onDelete = async (cb) => {
    const { id } = tweet
    if (id) {
      deleteTalk({
        variables: {
        ...
        onCompleted: (data) => {
          const { success, message } = data.deleteTalk
          if (cb) {
            cb()
          }
          if (success) {
  ...
  let finalText = text
  if (!full && tags.indexOf('blog') === -1 && text.length > 200) {
    ...
    <div className={styles.Container}>
      <div className={styles.MenuContainer}>
        ...
        <div className={styles.ActionButtonDelete}>
          {owner && <DeletionIcon onDelete={onDelete} />}
          {enableAvatar && (
      ...
      <div className={`${styles.ContentBody} max-w-md`} onClick={handleClick}>
        <ReactMarkdown
          remarkPlugins={[gfm]}
          rehypePlugins={[
            rehypeRaw,
            (options: Options) => rehypeHighlight({ ...options, ignoreMissing: true }),
          ]}
        >
          {finalText}
        </ReactMarkdown>
      ...
      <div className={styles.ItemFooter}>
        ...
        <div className={styles.MenuContainer}>
          <div className={styles.LikeIconContainer} onClick={handleLike}>
